mcmcMix <- function(y, X, model, prior = list(), mcmc, returnBurnin) {
  ...
  result <- list(
    beta = matrix(NA, M_thin + burnin, r),
    S = matrix(NA, M_thin + burnin, jk),
    eta = matrix(NA, M_thin + burnin, Lall),
    eta0 = matrix(
      NA,
      M_thin +
        burnin,
    ...
    mu = matrix(NA, M_thin + burnin, Lall),
    N_jl_matrix = matrix(
      NA,
      M_thin +
        burnin,
    ...
    N_j0_matrix = matrix(NA, M_thin + burnin, n_nom),
    sgma2 = rep(NA, M_thin + burnin)
  )
  ...
  for (m in 1:(M + burnin)) {
    ...
    BN <- sgma2 *
      backsolve(cholx, backsolve(cholx, diag(ncol(cholx)), transpose = TRUE))
